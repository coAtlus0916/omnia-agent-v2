...
- Shell 0.4.12 之后的日常开发不再为每次变更生成新 EXE 或 `releases/<patch>`。固定启动器先构建当前工作区，再由 0.4.12 的固定 Electron 宿主加载最新 `dist` 与 builtin Feature；只有用户明确要求形成新的不可变发布候选时才运行 Windows 打包。
