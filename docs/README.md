...
| 项目 | 当前状态 | 仍待完成 |
|---|---|---|
| Shell | `0.4.12` Remote-only：安全锁支持搜索、Omnia 真实所在部分折叠、右侧已选与全局关联锁；Core 单飞读取、CAS 持久化并冻结精确成员；固定宿主由启动器加载工作区热构建；内置 recording 0.3.0、create-associate 0.2.9 与 delete-elements 0.2.0 | 真实所在部分目录已现场只读采样；保存/冻结读回待发布后确认 |
