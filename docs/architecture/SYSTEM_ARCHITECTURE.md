...
- Remote Connector 只拥有凭据、Session、Pack binding、通用 Gate、声明式 Operation host 与 read-back transport。
...
- Connector/Bridge 禁止包含 Feature ID、Feature 版本或 Feature 业务分支；当前对 `src/connector`、`src/bridge`、`src/remote-connector` 的扫描未发现四个官方 Feature ID。
...
Remote Connector `0.3.36` 发布记录仍是 pending baseline；本页不把它当作当前 live 通过证据。
