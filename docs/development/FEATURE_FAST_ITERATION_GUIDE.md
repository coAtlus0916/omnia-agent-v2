...
6. 若 capability 依赖 Connector Operation，确认相同版本的 `.ofop` 已由 Local/Remote Operation host 装载；只在真实依赖存在时执行对应读/写测试。
...
- Connector Core/Transport/Supervisor 本身变化：重打 Local/Remote Connector；
...
| Local 链路 | Local Operation host 已装载 | 当前机器的真实 Local 路径可用 | Remote 同样可用 |
...
当前包管理器已经能安装、升级、回滚并投影文档，但尚没有通用 Feature Worker supervisor/action router，也没有把签名 Operation 包装载进 Local/Remote Connector host。“删除元素”0.1.0/0.1.1 因而只能测试包生命周期和 Worker 合同，不能测试真实 Omnia 删除。
...
3. Local Connector Operation host 的签名模块装载与 capability 报告；
4. Remote Connector Operation 的独立下发、side-by-side 与 capability 报告；
