...
- Migration 18 updates only DeepSeek profiles still equal to the former default Base URL/model pair to `https://provider.example.invalid` and `deepseek-v4-flash`. It does not rewrite user-customized profiles and leaves `api_key_ciphertext` byte-for-byte unchanged.
