...
版本：`0.4.12`
状态：Remote-only 发布源码；内置 recording 0.3.0、create-associate 0.2.9 与 delete-elements 0.2.0，均由 builtin bootstrap 自动安装/升级。0.4.12 固定宿主加载启动器构建的工作区代码；安全锁与 create-associate authority 只使用 Omnia 真实 `CustomWorkspaceGroup → CustomWorkspace.parentId`。Connector 仅执行固定读取和签名 Operation；Core/Worker 处理校验、规则与持久化。
