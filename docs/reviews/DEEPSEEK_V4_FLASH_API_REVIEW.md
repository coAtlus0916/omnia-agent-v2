...
- 新 DeepSeek profile 的规范 Base URL 使用 `https://api.deepseek.com`。现有 `https://api.deepseek.com/v1/` 可作为兼容输入继续接受，因为 DeepSeek 官方集成示例仍使用 `/v1/chat/completions`；不需要破坏用户已保存且验证成功的 profile。
...
- [Models & Pricing](https://api-docs.deepseek.com/quick_start/pricing/)：正式模型 ID、1M context、最大输出、Thinking、JSON Output、Tool Calls 和 Responses API 能力。
- [Lists Models](https://api-docs.deepseek.com/api/list-models/)：`GET /models` 返回 `deepseek-v4-flash` 与 `deepseek-v4-pro`。
- [Chat Completions API](https://api-docs.deepseek.com/api/create-chat-completion/)：`POST /chat/completions`、`thinking`、`reasoning_effort`、`response_format`、stream 与 response 字段。
- [Thinking Mode](https://api-docs.deepseek.com/guides/thinking_mode/)：默认启用、effort 映射、`reasoning_content` 和工具调用历史回传规则。
- [官方 WorkBuddy/CodeBuddy 接入示例](https://api-docs.deepseek.com/quick_start/agent_integrations/workbuddy/)：`/v1/chat/completions` 兼容 URL、Bearer 认证、模型 ID 和 `supportsImages=false`。
...
| OpenAI Base URL | `https://api.deepseek.com`；官方集成示例也展示 `/v1/chat/completions` |
| 模型发现 | `GET /models` |
...
| 默认 Base URL `https://api.deepseek.com/v1/` | 官方主文档采用根 URL，但 `/v1` 仍见于官方集成示例 | 新值使用根 URL；已保存 `/v1/` 不强制重写 |
