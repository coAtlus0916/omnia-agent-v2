...
Version 0.2.14 defines 11 check identities: `template_structure`, `required_fields`, `valid_values`, `unique_names`, `omnia_id_conflicts`, `infrastructure_links`, `infrastructure_rait`, `relationship_targets`, `workspace_presence`, `factors_considered_ai_review`, and `workspace_live`. States are passed/failed/warning/pending/skipped with a reason. Missing Connector binding or Workspace safety scope makes `omnia_id_conflicts`, `relationship_targets`, and `workspace_live` failed with the exact missing prerequisite; it never leaves the first two as generic pending. This is a retryable global blocker on the same Run and does not bypass the safety lock. Factors review is warning/not_evaluable when APP rows exist and skipped otherwise. `omnia_id_conflicts` passes only when the signed APP identity resolver proves an exact `create|resume|reuse` disposition; `skip`, ambiguity, drift, incomplete pagination or unavailable evidence blocks Return. `workspace_live` additionally requires an exact Engagement-bound Facet directory, real CustomWorkspace parent membership, a unique normalized name match, inclusion in the explicit Workspace safety scope, and exact APP/DB/OS/TOOL content/category identities from the current `Standardized Accounts List`. `tenantOrOrgId` is optional but remains an exact frozen string in Return scope equality and digests. Every Omnia/Core GUID is normalized to lowercase and must be a non-zero 8-4-4-4-12 hexadecimal value; UUID version and variant bits are not part of this contract.
