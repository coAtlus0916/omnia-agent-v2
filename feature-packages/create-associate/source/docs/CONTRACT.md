...
Version 0.2.24 defines the same 11 canonical checks and strict authority/GUID/Workspace contracts. GRA entity candidates comprise supported top-level direct IDs plus active risk scopes with an explicit canonical type equal to the expected object type; a scope that exposes `inkContentId`/`contentId` is eligible only when it equals the expected detail/query content ID. Untyped and other-type scopes are excluded, never inferred. Remaining GUIDs are deduplicated: one must equal the planned object, more than one blocks, and zero alone permits the strict current directory fallback. APP=Application/3, DB/OS=Infrastructure/4, TOOL=ITTool/5. Empty RAIT remains incomplete and cannot be inferred from directory data. All Application settings, authority, safety and receipt contracts remain unchanged.
