...
Version 0.2.45 keeps confirm/continue and every resulting command inside one authorized `omnia_mutation` Worker invocation. The Worker does not return an initial Surface and then continue mutating under an empty invocation identity. Every verified target updates Return progress and the durable execution checkpoint; interruption recovery still classifies pre-submit, completed, or uncertain state without replay. Risk/Control catalog settlement retains its 120-second/40-read upper bound and complete governed multiset requirement. Persistent remote identities remain bound to the current Pack's Workspace-scoped business identity. Authority, immutable intent, safety lock, mutation permit, readback and receipt contracts remain fail-closed.
