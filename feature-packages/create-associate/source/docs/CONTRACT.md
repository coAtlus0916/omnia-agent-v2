...
Version 0.2.23 defines the same 11 canonical checks and strict authority/GUID/Workspace contracts. GRA detail entity candidates are explicit: more than one is conflict, one must equal the planned object, and only zero permits a current live-directory fallback. That fallback requires one non-recycled, non-ambiguous merged row whose assessment ID, object ID, canonical name, Workspace and canonical type all equal the signed target/query. GRA reconcile signs both directory routes and independently verifies detail GRA ID/name/Workspace/type/ink-content. Missing directory fields, merge conflicts and multiple matches block; name alone is never evidence. Exact existing APP GRA identity accepts the planned live RAIT or empty RAIT; empty still requires the separate signed state PATCH/readback and directory RAIT is ignored. All Application settings, authority, safety and receipt contracts remain unchanged.
