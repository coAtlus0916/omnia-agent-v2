...
Version 0.2.41 returns confirm/continue after durable authority approval and background scheduling instead of holding the Surface request through the entire Return. One Worker may own only one active Run execution; duplicate starts reuse its live persisted Surface, while Worker restart recovery resumes the frozen plan from Core evidence. Every verified target updates Return progress and the durable execution checkpoint. Risk/Control catalog settlement has an explicit 120-second/40-read upper bound and still requires the complete governed relation multiset before any association. Persistent remote target identities remain bound to the current Pack's Workspace-bound object/GRA business identity plus the applicable field or relation ID; `rowKey` remains internal per-Run orchestration only. Authority, immutable intent, mutation, readback and receipt contracts remain fail-closed.
