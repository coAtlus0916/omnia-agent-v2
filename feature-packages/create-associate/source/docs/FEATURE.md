...
Version 0.2.28, sequence 30. The Feature retains the 0.2.27 incomplete-GRA recovery order and all prior identity, subtype, authority, safety and receipt contracts. Missing live GRA status/RAIT values are normalized to explicit `null` in the preview, and every deferred preview uses a complete typed desired object; no JavaScript `undefined` can enter the frozen preflight summary.
