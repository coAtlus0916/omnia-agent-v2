...
Version 0.2.31 keeps the four-plane boundary. The Worker query and frozen target remain unchanged. The signed Connector Operation treats generic search rows as partial endpoint-scoped projections, merges them by canonical GUID, rejects explicit conflicts, and delegates omitted list evidence to authoritative object detail plus Work Item-to-Workspace mapping. Core can therefore recover the exact existing DB/OS/Tool and reuse an existing relation without replaying mutation calls.
