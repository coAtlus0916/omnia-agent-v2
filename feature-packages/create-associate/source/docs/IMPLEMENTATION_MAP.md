...
Version 0.2.45 keeps the four-plane boundary. Worker awaits Return execution inside the originating authorized mutation action, while every target completion is persisted through existing Core Store ports. Worker remote target identities continue to bind Workspace + element kind + external element ID + derived GRA name and the relevant field/relation ID. Core authorization, UI and signed Operation contracts are unchanged.
