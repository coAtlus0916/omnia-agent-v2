...
Version 0.2.20 keeps the four-plane boundary: Renderer shows compact four-phase workflow and grouped progress; Worker freezes settings mode and orchestrates durable commands; Core alone proves prior product-owned object-create commits and binds settings mode and exact GRA name to immutable intent; the signed Connector Operation performs v4-compatible two-stage PATCH/GET/PATCH and final readback. GRA intent `externalId`, preview desired identity, create/reuse command name and readback name are the same canonical `GRA-${elementId}`. `recover_owned_create_bootstrap` requires both Core ownership proof and current exact empty live settings; ordinary `resume|reuse` remains token-gated. APP identity, object readback, and settings reads retain authoritative Work Item Facet Mapping.
