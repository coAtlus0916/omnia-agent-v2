...
Version 0.2.36 keeps the four-plane boundary. Core remains the durable intent and command authority. Worker mirrors a target into its execution-local progress map only after Core accepts verified readback evidence, so row-local and later global phases share the same completion view and cannot prepare the same frozen intent twice.
