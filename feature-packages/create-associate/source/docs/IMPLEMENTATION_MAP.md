...
Version 0.2.32 keeps the four-plane boundary. The Worker alone orchestrates the frozen Return phases while every read and mutation still crosses the signed Connector Operation. Risk Factors/documentation precede evaluation; evaluation uses bounded authoritative read polling; generated Risk/Control catalogs and associations follow only after every GRA is complete. Core receipts provide resume and no-replay semantics throughout.
