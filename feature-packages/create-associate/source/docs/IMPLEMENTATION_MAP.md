...
Version 0.2.48 keeps the four-plane boundary. Worker awaits Return execution inside the originating authorized mutation action and orchestrates the generated-Risk classification stage. Core validates each immutable classification command, persists every receipt and projects live progress without mutating the authorized Surface cache. Connector executes only the signed generated-Risk read/PATCH/reconcile routes with fresh concurrency evidence. Remote target identities continue to bind Workspace + element kind + external element ID + derived GRA name and the relevant field/relation ID.
