...
The user chooses an `.xlsx`; cancel creates no Run. A successful choice creates a durable Run and source artifact before Worker processing. Missing, conflicting, or ambiguous values become persistent issues. Editors save explicit user revisions with CAS. The compiled TemplateInstance remains an internal managed artifact used for provenance and Return planning; the Review page does not expose a download entry for it.
