...
Return confirmation renders immediately after durable authority approval and starts one real serial background execution. Refreshes display receipt-backed Core counts as targets complete; there is no timer animation or fabricated increment. Risk/Control generation may settle for at most 120 seconds with bounded backoff, after which the exact missing governed relations are reported and the Run fails closed.
