...
Version 0.2.103 preserves the base-GRA-only identity scope while closing four evidence gaps: exact GRA disposition enumeration, durable specs for reuse/read-only closures, receipt-payload equality inside Core projection, and exact DCNO inheritance/dependency inventories. The live verifier recomputes each command request digest from the durable command spec before accepting the receipt chain.
...
Python resolves every DB/OS relation token to one unique same-Workspace in-batch APP. The normalized row stores `inheritance.sourceApps` with every rowKey and elementId plus the deterministic RAIT result: any Higher source produces Higher; an all-Lower set produces Lower. Worker validates that frozen set against the prepared APP identities and emits one InfrastructureApplication relation per source. It does not recompute from an arbitrary remote row or fall back to one APP.
...
Return keeps the existing signed Operation, immutable intent, mutation permit, receipt, read-back and reconciliation contracts, but replaces whole-plan serial row traversal with the v4-proven dependency shape. The Worker uses a fixed default concurrency of three with a hard ceiling of four. An APP row's object/settings/GRA chain remains serial; each DB/OS row waits for its exact same-Workspace in-batch APP core node before starting, while independent rows may consume another slot. In every row, preflight, mutation, commit evidence and authoritative read-back remain strictly ordered and are never split across tasks.
