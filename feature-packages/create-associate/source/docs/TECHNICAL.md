...
## 0.2.45 authorized Return invocation lifetime
...
Local checks are recomputed from active rows while parser-origin structural/unmapped blockers are preserved. Inside a recognized section, only a row with no cell content is ignored as formatting; every row with user content becomes a source-located row/candidate set. An empty identity cell or fewer than two populated declared columns emits a deterministic parser-origin blocking issue, so later local recomputation cannot erase the structural evidence. Generated issues use explicit producer/code provenance and Run-scoped deterministic IDs. DB/OS requires exactly one in-batch, same-Workspace APP edge. Off-batch APP remains blocking. APP/DB/OS/Tool absent identities enter object-type-aware create-only preflight; recycle-bin and ambiguous identities fail closed. Removing a row performs no Connector call and marks relationship validation pending until explicit revalidation.
