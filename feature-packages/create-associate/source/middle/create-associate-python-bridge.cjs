...
function createSidecarNamespacePath(filename) {
...
class PythonSidecarBridge {
  ...
  async start() {
    ...
    const env = {
      ...
      OMNIA_PYTHON_PACKAGE_ROOT: createSidecarNamespacePath(this.packageRoot),
      OMNIA_PYTHON_TEMP_ROOT: createSidecarNamespacePath(this.tempRoot)
    };
    const child = spawn(this.pythonExecutable, ['-I', '-S', '-E', '-u', createSidecarNamespacePath(this.pythonEntry), '--stdio-rpc'], {
      cwd: createSidecarNamespacePath(this.tempRoot),
      env,
  ...
  async invoke(method, payload, options = {}) {
    ...
    const result = new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        void this.terminateCreateSidecarTree();
      }, timeoutMs);
  ...
  async close() {
    ...
    await this.terminateCreateSidecarTree();
  ...
  async terminateCreateSidecarTree() {
