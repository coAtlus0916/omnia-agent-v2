...
const RETURN_DEFAULT_CONCURRENCY = 4;
const RETURN_MAX_CONCURRENCY = 4;
...
const V8_SHA256 = 'CB44EE079D045564454F9A11015E6D2F91CF1C663608C9291CD00F0B1CF76F70';
...
function createFeatureWorker(dependencies) {
  ...
  async function waitForEvaluationComplete(connectorBinding, request) {
    ...
    for (let attempt = 0; attempt < 120; attempt += 1) {
      ...
      if (attempt < 119) await boundedDelay(1_000);
    }
  ...
  async function buildReturnPreparation(checkpoint, context) {
    ...
    if(!planIr||planIr.schemaVersion!=='omnia.create-associate.capability-plan-ir/v1'||planIr.parsedDigest!==checkpoint.parsed.semanticDigest
      ||planIr.governanceDigest!==governance.semanticDigest||planIr.semanticDigest!==planIrDigest||!Array.isArray(planIr.rows)){
    ...
    handleAction: async (input) => {
      ...
      if (input?.actionId === 'reconcile-return') {
        ...
        if(reconcileOperation===RETURN_OPERATIONS.objectIdentityResolve){
          ...
          if(spec.waitForEvaluationComplete) observed=await waitForEvaluationComplete(binding,readRequest);
            ...
            const attempts=Math.max(1,Math.min(RETURN_READBACK_MAX_ATTEMPTS,Number(spec.readbackPolicy?.maxAttempts||1)));
            for(let attempt=0;attempt<attempts;attempt+=1){
              observed=await invoke(spec.readOperation,binding,readRequest);
              if(observed?.verified===true||attempt===attempts-1)break;
              await boundedDelay(Math.max(0,Math.min(1_000,Number(spec.readbackPolicy?.delayMs||0))));
            }
          }
      ...
      if (input?.actionId === 'confirm-return' || input?.actionId === 'continue-return') {
        ...
        if(input.actionId==='continue-return'){
          ...
          const allVerified=completionProgress.length>0&&completionProgress.every((item)=>String(item.state||'')==='verified'
            ||['readback_verified','closed_not_applied'].includes(String(item.command_state||'')));
          if(allVerified){
            await store.call('validateReturnAuthority',{runId,connectorBinding:input.context.connectorBinding,safetyLock:input.context.safetyLock});
        ...
        try {
          ...
          await runDependencyPipelineRows(ordered,async(row)=>{
            ...
            if (hasFrozenStage(row,'app_scoring')) {
              ...
              for (const factorIntent of factorIntents) {
                ...
                const factorPreflight = await invoke(RETURN_OPERATIONS.factorPreflight, binding, { target, query: factorQuery });
          ...
          // signed EvaluationComplete read-back. Frozen identities remain exact.
            ...
            const requiredRelations=hasFrozenStage(row,'risk_control')?returnIntents.riskControlRelations:[];
            ...
            let catalog=pendingRelations.length?await waitForCompleteRiskControlCatalog(binding,catalogRequest,requiredRelations,mode):null;
              ...
              const result=existingRisk.verified===true
                ...
                :await verifiedMutation({targetKey,target,preflightOperation:RETURN_OPERATIONS.riskPreflight,
                  ...
                  mutationOperation:RETURN_OPERATIONS.riskWrite,commandKind:'associate_risk_control',mutationPayload:{
                    engagementId:binding.engagementId,workspaceId:row.workspaceId,riskAssessmentId:graId,riskName:targetSpec.riskName,
                  ...
                  readOperation:RETURN_OPERATIONS.riskRead,readRequest:{target,query:riskQuery},readbackPolicy:{maxAttempts:RETURN_READBACK_MAX_ATTEMPTS,delayMs:1_000},verify:(observed)=>observed.verified===true});
          ...
          const unresolved=finalProgress.filter((item)=>String(item.state||'')!=='verified'
            &&!['readback_verified','closed_not_applied'].includes(String(item.command_state||'')));
