...
function validationPresentation(parsed,live={}){
  ...
  const checkFailed=(id)=>errors.some((candidate)=>inferredCheck(candidate)===id);const workspaceKeys=new Set((parsed.candidates||[]).filter((candidate)=>candidate.rawFieldKey==='Omnia工作区').map((candidate)=>candidate.fieldKey));const workspaceMissing=errors.some((candidate)=>workspaceKeys.has(candidate.fieldKey)||String(candidate.fieldKey).endsWith(`.missing.${digest('Omnia工作区')}`)); const liveCheck=(id)=>live[id]||{state:'pending',reason:'未执行实时校验，不视为通过。'};
...
function createFeatureWorker(dependencies) {
  ...
  async function runReviewLiveValidation(checkpoint,context){
    ...
    const unavailable={omnia_id_conflicts:{state:'pending',reason:'当前未执行 APP 身份/回收站及非 APP 活动对象检查。'},relationship_targets:{state:'pending',reason:'当前未执行关系目标类型实时检查。'},workspace_live:{state:'failed',reason:'Remote Connector 断开；这是可重试的全局错误，可在原 Run 上重新校验。'}};
    const binding=context?.connectorBinding,safety=context?.safetyLock;if(!binding?.connectorId||Number(binding.sessionGeneration)<1||!binding.engagementId){const reason='当前没有可用的 Remote Connector binding；连接后可在原 Run 重试。';checkpoint.parsed.issues.push(liveIssue('LIVE.WORKSPACE_UNAVAILABLE','global.workspace_live','contract_mismatch','blocking',reason,'workspace_live'));return{...unavailable,workspace_live:{state:'failed',reason}};}if(!Array.isArray(safety?.workspaceIds)||!safety.workspaceIds.length){const reason='请启用当前 Pack Workspace 安全范围后在原 Run 重新校验。';checkpoint.parsed.issues.push(liveIssue('LIVE.SAFETY_SCOPE_UNAVAILABLE','global.workspace_live','contract_mismatch','blocking',reason,'workspace_live'));return{...unavailable,workspace_live:{state:'failed',reason}};}
    try{const query=authorityRequest(checkpoint,context).query;const authority=await invoke(RETURN_OPERATIONS.authority,binding,{allowedWorkspaceIds:safety.workspaceIds,query});const byName=new Map((authority.workspaces||[]).map((item)=>[String(item.name).normalize('NFKC'),item.workspaceId]));const missing=query.workspaceNames.filter((name)=>!byName.has(String(name).normalize('NFKC')));if(missing.length){checkpoint.parsed.issues.push(liveIssue('LIVE.WORKSPACE_NOT_FOUND','global.workspace_live','contract_mismatch','blocking',`Omnia 工作区实时不存在或不在安全范围：${missing.join(', ')}`,'workspace_live'));return{...unavailable,workspace_live:{state:'failed',reason:`未精确匹配：${missing.join(', ')}`}};}
    ...
    }catch(error){const reason=`实时校验失败（可在原 Run 重试）：${String(error.message||error)}`;checkpoint.parsed.issues.push(liveIssue('LIVE.VALIDATION_FAILED','global.workspace_live','contract_mismatch','blocking',reason,'workspace_live'));return{...unavailable,workspace_live:{state:'failed',reason}};}
  }
  ...
  return Object.freeze({
    ...
    handleAction: async (input) => {
      ...
      if (['apply-revisions','remove-batch-row','revalidate-all','back-to-upload'].includes(input?.actionId)) {
        ...
        const revisions=input.actionId==='apply-revisions'?(input.payload?.revisions||[]):[];if(!Array.isArray(revisions))fail('REVISION.BATCH_INVALID','Review revisions must be an array.');const derivedRevisions=[];
      ...
      return {
        surfacePatch: {
          ...
          scopes:progress.scopes,items:progress.items,workflow:progress.workflow,progress:validation.progress,issues:validation.issues,artifacts: [{
            artifactId: output.artifactId, kind: 'template_instance', name: `create-associate-${runId}.xlsx`,
            sha256: output.sha256, sizeBytes: output.sizeBytes, available: true, reason: ''
          }],
