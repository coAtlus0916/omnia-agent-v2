...
try:
    ...
    PARSED_SCHEMA = _engine_module.PARSED_SCHEMA
    derive_gra_name = _engine_module.derive_gra_name
...
def _requested_family(kind: str, content: str) -> str:
    ...
    aliases = {
        ...
        "OS": {"GENERIC": ("generic", "generic operating system"), "UNIX": ("unix",), "WIN": ("win", "windows"), "AD": ("ad", "active directory")},
        "TOOL": {"TICKET": ("工单工具", "ticketing tool"), "IDENTITY": ("身份和访问管理工具", "identity & access management tool"), "MIGRATION": ("代码迁移工具", "code migration tool")},
...
def _risk_classifications(*, fields: list[dict], selected_relations: list[dict], kind: str, content: str, mode: str) -> list[dict]:
    ...
    for field in fields:
        ...
        declared_field = re.search(rf"(?:^|;)\s*{mode}\s*=\s*(Higher|Lower|ClassificationNA)(?:\s*;|$)", str(field.get("validationRule") or ""))
        require(declared_field is not None, "PLAN.RISK_CLASSIFICATION_GOVERNANCE_INVALID", f"Risk {risk_number}/{mode} has no explicit field-level classification rule.")
        classification = declared_field.group(1)
        require(not any(_yes(relation.get(f"linkRequired{mode}")) for relation in field_relations) or classification == mode,
                "PLAN.RISK_CLASSIFICATION_GOVERNANCE_CONFLICT", f"Risk {risk_number}/{mode} has a required Control link but is not classified {mode}.")
