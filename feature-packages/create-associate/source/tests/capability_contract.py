...
from engine import _is_candidate_data_row, parse_workbook, validate_ir  # noqa: E402
...
def assert_registry(case: dict, governance: dict) -> None:
    ...
    require(registry["OS"].get("pendingRecordingContentValues") == [{"inputValue": "AD", "expectedOmniaContentName": "", "status": "blocked_pending_recording"}], "AD pending-recording boundary drifted.")
    require(registry["TOOL"].get("pendingRecordingContentValues") == [{"inputValue": "代码迁移工具", "expectedOmniaContentName": "", "status": "blocked_pending_recording"}], "Code-migration Tool pending-recording boundary drifted.")
    ...
    require(oracle.get("contentName") == "Oracle eBusiness Suite" and oracle.get("higherRelations") == 11
            and oracle.get("lowerRelations") == 7 and oracle.get("catalogRelations") == 12,
            "Oracle EBS governed content/scope counts drifted.")
