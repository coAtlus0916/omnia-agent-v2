...
| 中台 | `middle/worker.cjs` + `middle/python-bridge.cjs` + `python/engine.py` | Worker 冻结完整图并掌控 Core/Operation/证据；托管 CPython 仅从冻结图与持久 ledger 决定 ready/skip/terminal | Python 不访问 Core/Store/Connector/网络；Worker 不重放不确定 mutation；无 JS 调度回退 |
