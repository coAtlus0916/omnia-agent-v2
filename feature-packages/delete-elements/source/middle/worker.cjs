...
const { createPythonSidecarBridge } = require('./delete-elements-python-bridge.cjs');
...
function createFeatureWorker(ports) {
  ...
  async function checkpointFinalCatalog(plan, terminal) {
    if (!['succeeded', 'failed'].includes(String(terminal || ''))) {
      fail('DELETE.FINAL_CATALOG_STATE_INVALID', 'Final authoritative recapture has no frozen scheduler terminal state.');
    }
    if (plan.state === 'uncertain' && plan.uncertain && plan.uncertain.phase === 'final_catalog') {
      if (String(plan.uncertain.terminal || '') !== terminal) {
        fail('DELETE.FINAL_CATALOG_STATE_INVALID', 'Final authoritative recapture terminal state changed after checkpoint.');
      }
      return plan;
    }
    plan.state = 'uncertain'; plan.stateVersion += 1;
    plan.uncertain = { phase: 'final_catalog', terminal };
    plan.finalVerification = { schemaVersion: 'omnia.delete-final-catalog-verification/v1', state: 'pending',
      attemptedAt: new Date().toISOString() };
    await save(plan); return plan;
  }
  async function finishAfterFinalRecapture(plan, context, terminal) {
    await checkpointFinalCatalog(plan, terminal);
    let recaptured;
