...
try {
  ...
  process.env.OMNIA_MANAGED_PYTHON_ENTRY = path.join(installedRoot, 'python', 'engine.py');
  process.env.OMNIA_FEATURE_PACKAGE_ROOT = installedRoot;
  process.env.OMNIA_FEATURE_TEMP_ROOT = pythonTemp;
  const require = createRequire(import.meta.url);
  const bridgeModule = require(path.join(installedRoot, 'middle', 'python-bridge.cjs'));
