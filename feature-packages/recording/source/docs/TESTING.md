...
19. 0.4.19→0.4.20 必须从磁盘加载已安装的 legacy 通用 Python 路径包，但只允许新候选使用 `recording-python-bridge.cjs`/`recording-engine.py`。测试必须证明该 grandfathering 仅用于读取 source head，0.4.19 的 frozen-input plan 与 Processing Run 保持不变，handoff ledger 精确暂存到 0.4.20；新安装包若退回通用 Python 路径仍须失败。
