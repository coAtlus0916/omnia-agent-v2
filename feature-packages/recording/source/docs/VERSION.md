...
- Python 边界改为 Feature 唯一入口 `middle/recording-python-bridge.cjs` 与 `python/recording-engine.py`；运行合同显式声明 Core `appendEvidence` Store port，禁止与其他 Feature 共用通用 bridge/engine 路径或私有数据口。
- frozen-input 恢复来源增加 0.4.19；签名 Operation 的 legacy digest allowlist 仍只精确覆盖 0.4.16/0.4.17/0.4.18。0.4.19 已拥有同一稳定 resource-owner identity，0.4.19→0.4.20 必须走 owner identity + ABI fingerprint + 单调 sequence 证明，不能把版本号冒充 legacy digest。
- Operation handler 必须与 0.4.18/0.4.19 的 4,598 bytes 完全一致，SHA-256 固定为 `43b48912e09d66a413e2af831fbc40ab22df8a09119bc8ca40ceb8b4ada47ef7`；打包器在签名前对漂移失败关闭。
- 0.4.19/sequence 32 候选保持不可变；本版本只允许从独立冻结提交生成新的不可变 `.ofp/.ofop`，不覆盖旧候选。
