...
- `omnia.workpaper.phase2.writeback.v1`：只接受 Core durable command、幂等 delivery identity、冻结 plan 和精确 Control 目标；`valueKind=editor` 必须是录制证明的 Omnia 富文本 JSON（`editorData`、`suggestionsData`、`trackChangesEnableFlagInEditor`、`plainText`），拒绝裸文本；在 PATCH 前逐字验证冻结的 TestOfDesign 原始值与最新权威详情一致，PATCH 后立即读回完整编辑器 JSON 并返回逐字段 ledger。
