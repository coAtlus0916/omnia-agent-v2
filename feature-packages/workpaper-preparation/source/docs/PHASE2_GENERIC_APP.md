...
用户也可以完全不上传参数表和制度资料。上传步骤开始时即显示“跳过”；仅在两类资料都尚未上传时可用。点击后 Worker 记录真实的空资料状态，不调用制度 AI，使用预置母版生成后台结果并保留全部原 `【占位符】`，随后进入同一个“确认回传”步骤。跳过不会自动提交到 Pack，也不会绕过实时 Control 读取、签名写回、并发门禁或逐字段读回。生成模板后若 Connector Session 重建，只允许尚未创建 Return intent、稳定 Pack/Engagement/Workspace 权威完全一致且安全锁已经重新验证的草稿换绑；任何已开始或 uncertain 的写回计划仍失败关闭。
