...
function sameSafety(current, frozen) {
  if (Number(current.stateVersion) !== Number(frozen.stateVersion)
    ...
    || canonical(current.workspaceIds) !== canonical(frozen.workspaceIds)
    || canonical(current.globalSectionIds) !== canonical(frozen.globalSectionIds)
    || canonical(current.globalWorkspaceIds) !== canonical(frozen.globalWorkspaceIds)) {
...
function normalizeDirectory(value, b, s) {
  ...
  if (workspaces.length !== s.workspaceIds.length || canonical(unique(workspaces.map((item) => item.id))) !== canonical(s.workspaceIds)) {
