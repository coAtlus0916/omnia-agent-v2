...
def recorded_new_elements_cells(
    ...
    """One real dependency-closed DCNO/HANA/EBS/AD/code-migration acceptance batch."""
    suffix = "H" if mode == "Higher" else "L"
    hana = f"{prefix}-HANA-{suffix}"
    ebs = f"{prefix}-EBS-{suffix}"
    ad = f"{prefix}-AD-{suffix}"
    network = f"{prefix}-DCNO-{suffix}"
    migration = f"{prefix}-MIGRATION-{suffix}"
    description = (
        "Processes explicitly identified key financial data."
        if mode == "Higher"
        else "Contains no key data according to the supplied assessment text."
    ...
    cells = {
        "B23": hana, "C23": "SAP S/4 HANA", "D23": mode, "E23": description, "F23": workspace,
        "B24": ebs, "C24": "Oracle EBS", "D24": mode, "E24": description, "F24": workspace,
        "B35": ad, "C35": "AD", "D35": workspace, "E35": hana,
        "B40": network, "C40": "网络", "D40": workspace, "E40": hana,
        "B46": migration, "C46": "代码迁移工具", "D46": mode, "E46": workspace, "F46": ebs,
    }
    ebs_required = 11 if mode == "Higher" else 7
    expected = {
        hana: {"kind": "APP", "rait": mode, "dependencies": []},
        ebs: {"kind": "APP", "rait": mode, "dependencies": [], "catalog": 12, "required": ebs_required},
...
def validate_with_installed_package(
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("create_associate_engine", engine_path)
        require(spec is not None and spec.loader is not None, f"Installed engine entry is invalid: {engine_path}")
