...
const version = '0.2.50'; const sequence = 52;
...
const runtimeContract={schemaVersion:'omnia.feature-runtime-contract/v1',featureId:'omnia.create-associate',featureVersion:version,
  inputs:['staged_xlsx_artifact','upload_confirmation','background_validation','issue_revisions','ai_provider_review','return_confirmation','connector_binding','workspace_safety'],outputs:['template_instance_xlsx','surface_patch','confirmation_card','managed_object_revisions','managed_relation_revisions'],
  events:['artifact.staging_replaced','workbook.upload_confirmed','run.transition','run.offline_crash_recovered','run.restart_requested','return.confirmation_invalidated','return.confirmed_in_comments','return.readback_verified','return.uncertain'],
  errors:['WORKBOOK.*','GOVERNANCE.*','AI.REVIEW.*','RETURN.*','CONNECTOR.RESPONSE_LOST'],
  storePorts:['createRun','readArtifactBytes','readManagedAssetBytes','commitArtifact','transitionRun','recordFieldRevisions','recordIssues','loadRunReview','commitReviewValidation','returnRunToReview','restartRun','proveOwnedCreatedObject','prepareReturnIntent','approveReturnIntent','prepareReturnCommand','freezeReturnEvidenceSpec','recordReturnEvidence','projectVerifiedReturn','recordBootstrapCapabilityEvidence','getCapabilityEvidenceState','finishReturn','savePlan','loadPlan','openPythonArtifactHandle','createPythonJsonInputHandle','createPythonOutputHandle','readPythonJsonHandle','commitPythonOutputHandle','releasePythonArtifactHandles'],
