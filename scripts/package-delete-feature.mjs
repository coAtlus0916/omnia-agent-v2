...
async function buildVersion(version, sequence) {
  ...
  const pythonMemberPaths = ['python/delete-elements-engine.py'];
  const runtimeContract = {
    ...
    pythonSidecar: {
      ...
      protocol: 'omnia.python-sidecar-rpc/v1', bridgePath: 'middle/delete-elements-python-bridge.cjs', entryPath: 'python/delete-elements-engine.py', members: pythonMemberPaths,
  ...
  const implementationContract = {
    ...
    planes: {
      surface: ['frontend/surface.json'], worker: ['middle/worker.cjs', 'middle/delete-elements-python-bridge.cjs', ...pythonMemberPaths],
  ...
  const testIds = ['package-structure-static-check', 'final-authoritative-catalog-absence', 'signed-operation-readback-contract', 'recorded-gra-deletion-contract', 'dcno-parameterized-delete-contract', 'mixed-type-authoritative-catalog', 'core-terminal-truthfulness', 'bounded-authoritative-catalog-loading',
    ...
    'managed-python-preparation-compiler', 'compiler-output-tamper-fail-close', 'final-catalog-restart-resume'];
...
const engine=read('python/delete-elements-engine.py');
const bridgeSource=read('middle/delete-elements-python-bridge.cjs');
...
const worker=require(path.join(root,'middle','worker.cjs'));
if(manifest.featureId!=='omnia.delete-elements'||manifest.version!=='${version}'||runtime.pythonSidecar?.version!=='3.13.14'||runtime.pythonSidecar?.members?.join(',')!=='python/delete-elements-engine.py')throw new Error('signed runtime identity failed');
if(!implementation.planes?.worker?.includes('middle/delete-elements-python-bridge.cjs')||vectors.vectors?.length!==17||!vectors.vectors?.every((item)=>item.inputRef==='tests/self-test.cjs'&&item.expected==='pass')||!vectors.vectors?.some((item)=>item.testId==='feature-local-delete-plan-surface')||!vectors.vectors?.some((item)=>item.testId==='bounded-plan-preflight-concurrency')||!vectors.vectors?.some((item)=>item.testId==='staged-plan-preparation-checkpoint')||!vectors.vectors?.some((item)=>item.testId==='grouped-relation-mutation-readback')||!vectors.vectors?.some((item)=>item.testId==='reopen-authoritative-catalog')||!vectors.vectors?.some((item)=>item.testId==='no-comments-projection')||!vectors.vectors?.some((item)=>item.testId==='managed-python-preparation-compiler')||!vectors.vectors?.some((item)=>item.testId==='compiler-output-tamper-fail-close')||!vectors.vectors?.some((item)=>item.testId==='final-catalog-restart-resume')||surface.status!=='loading'||surface.selectionBrowser?.layout?.mode!=='fixed_footer_split'||surface.actions?.find((item)=>item.actionId==='bootstrap-authoritative-catalog')?.presentation!=='background'||surface.actions?.find((item)=>item.actionId==='continue-delete-plan-preparation')?.presentation!=='background'||surface.actions?.find((item)=>item.actionId==='continue-delete-plan-preparation')?.visible===false||surface.actions?.find((item)=>item.actionId==='continue-delete-plan-preparation')?.effect!=='local_state_write'||surface.actions?.find((item)=>item.actionId==='retry-delete-plan-preparation')?.effect!=='local_state_write'||surface.lifecycle?.schemaVersion!=='omnia.declarative-feature-surface-lifecycle/v1'||surface.lifecycle?.onReopenActionId!=='refresh-on-reopen'||surface.actions?.find((item)=>item.actionId==='refresh-on-reopen')?.visible!==false||surface.actions?.find((item)=>item.actionId==='refresh-on-reopen')?.presentation!=='background')throw new Error('deletion package declaration failed');
...
if(!workerSource.includes('INTERACTIVE_CATALOG_TIMEOUT_MS = 90_000')||!workerSource.includes('PLAN_PREFLIGHT_CONCURRENCY = 8')||!workerSource.includes('PLAN_PREPARATION_BATCH_SIZE = 8')||!workerSource.includes("state: 'preparing'")||!workerSource.includes('settlePreparationBatch')||!workerSource.includes('loadCatalogSnapshot')||!workerSource.includes('continuePlanPreparation')||!workerSource.includes('compilePreparationGraph')||!workerSource.includes('verifyCompiledPreparation')||!workerSource.includes('verifiedCheckpointCompilation')||!workerSource.includes('compilationDigest')||workerSource.includes('derivePreparationGraph')||!workerSource.includes('checkpointFinalCatalog')||!workerSource.includes("phase: 'final_catalog'")||!workerSource.includes("toState: 'cancelled'")||!workerSource.includes('planSurface(plan)')||workerSource.includes(forbiddenProjectionKey)||workerSource.includes(['function ','card('].join(''))||!workerSource.includes('catalogReadInFlight')||!workerSource.includes("input.actionId === 'refresh-on-reopen'")||!workerSource.includes('latestPlanForReopen')||!workerSource.includes('frozenPlanRefreshFailureProjection'))throw new Error('staged Python-compiled bounded plan preflight, restart-safe final catalog, Feature-only Surface, no-Comments, or reopen catalog contract failed');
...
`;
  ...
  const featurePackage = envelope({
    ...
    files: [
      ...
      file('middle/delete-elements-python-bridge.cjs', await readFile(path.join(source, 'middle', 'delete-elements-python-bridge.cjs'))),
      file('python/delete-elements-engine.py', await readFile(path.join(source, 'python', 'delete-elements-engine.py'))),
