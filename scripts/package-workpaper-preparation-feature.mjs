...
const operations = [
  ...
  {
    operationId: 'omnia.workpaper.phase2.writeback.v1', effect: 'omnia_mutation',
    requestSchema: 'omnia.workpaper.phase2-writeback-request/v1', responseSchema: 'omnia.workpaper.phase2-writeback-response/v1',
    enabledByDefault: false, grantsMutationPermit: false, routes: [
      ...graContextRoutes('writeback'),
      route('writeback-control-detail', 'GET', '/rapr/v0/engagements/{engagementId}/controls/{controlId}', [{ name: 'controlId', type: 'guid' }]),
      route('writeback-patch', 'PATCH', '/rapr/v0/engagements/{engagementId}/controls/{controlId}', [{ name: 'controlId', type: 'guid' }], 'signed_json'),
      route('writeback-readback', 'GET', '/rapr/v0/engagements/{engagementId}/controls/{controlId}', [{ name: 'controlId', type: 'guid' }])
    ]
  }
];
...
const phase2Fields = JSON.parse(await readFile(path.join(source, 'docs', 'PHASE2_GENERIC_APP_FIELDS.json'), 'utf8'));
const phase2Template = JSON.parse(await readFile(path.join(source, 'managed', 'phase2-template-data.json'), 'utf8'));
...
const runtimeContract = {
  ...
  aiReviewCapabilities: [{ capabilityId: 'phase2-policy-resolution/v1',
    requestSchemaVersion: 'omnia.feature-ai-review-request/v1', maxRequestBytes: 1024 * 1024 }],
...
const implementationContract = { schemaVersion: 'omnia.feature-implementation-map/v1', featureId: 'omnia.workpaper-preparation', featureVersion: version,
  planes: { surface: ['frontend/surface.json'], worker: ['middle/worker.cjs', 'middle/workpaper-preparation-python-bridge.cjs', 'python/workpaper-preparation-engine.py', 'python/workpaper_workbook.py', 'python/ooxml.py', 'python/policy_extract.py', 'python/policy_resolve.py', 'python/canonical.py', 'python/errors.py'],
...
const actions = [
  ...
  { actionId: 'select-elements', label: '下一步', effect: 'local_state_write', enabled: true, reason: '', selectionMode: 'single', dependencies: ['remote_connector', 'safety_lock'] },
  { actionId: 'upload-filled-workbook', label: '上传填写好的参数表', effect: 'local_state_write', enabled: false, reason: '请先生成填写件，再上传填写好的参数表。', selectionMode: 'none', dependencies: ['remote_connector', 'safety_lock'], input: { kind: 'open_file', accept: ['.xlsx', '.xlsm'], label: '选择填写好的参数表' } },
  ...
  { actionId: 'skip-materials', label: '跳过', effect: 'local_state_write', enabled: false, reason: '请先生成填写件模板。', presentation: 'upload', selectionMode: 'none', dependencies: ['remote_connector', 'safety_lock'] },
...
const surface = {
  ...
  description: '选择一个 Generic Application GRA，生成填写件模板并上传材料，确认后激活 OE Tab 并写回 Control。', density: 'compact',
  ...
  workflow: { revision: 1, currentStepId: 'select', steps: [
    ...
    { stepId: 'upload', label: '上传材料', state: 'pending', detail: '上传填写件与制度资料，或跳过并保留母版占位内容' },
...
const handler=handlerMember&&Buffer.from(handlerMember.contentBase64,'base64').toString('utf8');const worker=read('middle/worker.cjs'),engine=read('python/workpaper-preparation-engine.py'),workbook=read('python/workpaper_workbook.py');
...
if(!worker.includes("finishReturn', { runId: plan.runId, outcome: 'uncertain'")||!worker.includes("operationId: OPERATIONS.reconcile")
  ...
  ||!worker.includes('generateWorkpaper(plan, context)')||!worker.includes('applyReplacementFields(plan, context')||!worker.includes('commitResolvedWorkbook(plan, context)')||!worker.includes('confirmWriteback(plan, context')||!worker.includes('commitStandaloneArtifact')
  ||!surface.actions.some((item)=>item.actionId==='upload-filled-workbook'&&item.input?.kind==='open_file'&&item.input?.label==='选择填写好的参数表')||!surface.actions.some((item)=>item.actionId==='upload-policy'&&item.input?.kind==='open_file'&&item.input?.multiple===true&&item.input?.directory===true)||!surface.actions.some((item)=>item.actionId==='confirm-writeback'&&item.effect==='omnia_mutation')
  ...
  ||!worker.includes('skipMaterials(plan, context)')||!worker.includes("templateMode: 'master_placeholders'")||!worker.includes('draftAuthority(plan, context')
...
const worker = (await readFile(path.join(source, 'middle', 'worker.cjs'), 'utf8'))
  .replaceAll('__FEATURE_VERSION__', version)
  .replaceAll('__PHASE2_FIELDS__', JSON.stringify(phase2Fields.fields))
  .replaceAll('__PHASE2_TEMPLATE__', JSON.stringify(phase2Template));
const featurePackage = envelope({ product: 'omnia-feature', packageId: featureManifest.featureId,
  keyId: 'omnia-v5-official-feature-2026-01', privateKey: featurePrivateKey, files: [
    ...
    file('python/workpaper_workbook.py', await readFile(path.join(source, 'python', 'workpaper_workbook.py'))),
    file('python/ooxml.py', await readFile(path.join(source, 'python', 'ooxml.py'))),
    file('python/policy_extract.py', await readFile(path.join(source, 'python', 'policy_extract.py'))),
    file('python/policy_resolve.py', await readFile(path.join(source, 'python', 'policy_resolve.py'))),
    file('python/canonical.py', await readFile(path.join(source, 'python', 'canonical.py'))),
    file('python/errors.py', await readFile(path.join(source, 'python', 'errors.py'))),
