...
assert.equal(stable.url, `https://download.labcaspian.com/files/v5-remote-connector/releases/${REMOTE_CONNECTOR_VERSION}/${packageName}.zip`);
assert.equal(stable.size, fs.statSync(archivePath).size);
assert.equal(stable.sha256, sha256File(archivePath));
assert.equal(REMOTE_CONNECTOR_UPDATE_MANIFEST_URL, 'https://download.labcaspian.com/files/v5-remote-connector/stable.json');
