...
type DurableRegistrationTransaction = {
  ...
  phase: 'committed' | 'finalized' | 'aborted';
  committedAt: string;
  updatedAt: string;
...
export class OperationHost {
  ...
  maintenanceSnapshot(): {
    ...
    const pendingRegistrationTokens = new Set([
      ...
      ...[...this.durableRegistrations.entries()].filter(([, transaction]) => transaction.phase === 'committed').map(([token]) => token)
    ]);
  ...
  register(input: OperationRegistrationCommand, binding: ConnectorBinding): OperationRegistrationResult {
    ...
    if (!resourceOwner && previousOwners.length > 0) {
      throw new Error('Operation package replacement requires an explicit signed resource owner handoff declaration.');
    }
    for (const previous of previousOwners) {
    ...
    this.preparedRegistrations.set(registrationToken, {
  ...
  private durableRegistrationResult(durable: DurableRegistrationTransaction): OperationRegistrationResult {
    return {
      ...
      registrationState: durable.phase === 'aborted' ? 'aborted' : 'committed',
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (value.schemaVersion !== 'omnia.operation-registration-ledger/v1'
          ...
          || !['committed', 'finalized', 'aborted'].includes(value.phase)
          || !Number.isFinite(Date.parse(value.committedAt)) || !Number.isFinite(Date.parse(value.updatedAt))
          || (value.phase === 'committed' ? value.expiresAt !== null : !Number.isFinite(expiry))
