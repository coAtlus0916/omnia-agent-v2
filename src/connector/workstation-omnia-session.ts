...
const DEFAULT_HOME = 'https://omnia.example.invalid/';
  ...
  async connect(): Promise<ConnectorConnection> {
    ...
    if (observed.status === 'waiting_authorization' && isAllowedOmniaUrl(page.url())) {
      ...
      if (isGuid(engagementId)) {
        ...
        // traffic. Wait only for a newly captured header; do not read browser
        // storage, synthesize credentials, reload, navigate or replace Page.
        await this.waitForAuthorization(
          page,
          engagementId,
          this.authorizationCaptureEpoch,
          EXPLICIT_CONNECT_AUTHORIZATION_WAIT_MS
        );
