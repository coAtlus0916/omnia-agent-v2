...
const connectorSupportsDurableDelivery = (value: string): boolean => {
  const match = value.match(/^(\d+)\.(\d+)\.(\d+)$/u);
  return Boolean(match && (Number(match[1]) > 0
    || Number(match[2]) > 3
    || (Number(match[2]) === 3 && Number(match[3]) >= 36)));
};
...
export class RemoteConnectorTransport implements ConnectorTransport {
  ...
  private connectorStateEpoch = 0;
  private protocolAdmissionIdentity: {
    stateEpoch: number;
  ...
  private protocolAdmissionTask: Promise<void> | null = null;
  ...
  async start(): Promise<void> {
    ...
    if (nextIdentity !== this.configIdentity) {
      ...
      this.invalidateProtocolAdmission();
    }
  ...
  private handleMessage(text: string): void {
    ...
    if (envelope.kind === 'state') {
      // Bridge pairing generation does not change when a Connector process is
      // replaced. Every authenticated state publication therefore starts a
      // fresh local epoch and invalidates any older, possibly late handshake.
      this.invalidateProtocolAdmission();
      ...
      this.events.emit('state');
      if (this.connectorOnline && connectorSupportsDurableDelivery(this.connectorVersion)) {
        this.requestProtocolAdmission();
      }
      return;
    }
    ...
    if (!response.ok && response.error?.code === 'REMOTE.PROTOCOL_ADMISSION_REQUIRED'
      && !response.delivery) {
      // This platform error is emitted by the authenticated Worker before the
      // command gate, rollback barrier, activity ledger, or Feature handler.
      // It is the only safe same-request retry signal across a Worker swap.
      this.invalidateProtocolAdmission();
      pending.reject(new AppError(
        'REMOTE.PROTOCOL_ADMISSION_REQUIRED',
        response.error.message || 'Connector Worker protocol admission must be renewed.',
        true
      ));
      this.requestProtocolAdmission();
      return;
    }
    if (response.ok || pending.deliveryWitness) {
      ...
      if (pending.deliveryWitness) {
        const delivery = response.delivery;
        const responseWithoutDelivery = { ...response };
        delete responseWithoutDelivery.delivery;
        if (!delivery || delivery.schemaVersion !== 'omnia.connector-wire-delivery/v1'
          || !/^[a-f0-9]{64}$/u.test(delivery.resultDigest)
          || !/^[a-f0-9]{48}$/u.test(delivery.executionGeneration)
          || !Number.isSafeInteger(delivery.sessionGeneration) || delivery.sessionGeneration <= 0
          || connectorResultDigest(responseWithoutDelivery) !== delivery.resultDigest) {
          pending.reject(new AppError('REMOTE.DELIVERY_WITNESS_INVALID', 'Remote Connector delivery witness failed exact digest validation.'));
          return;
        }
        pending.resolve({
          ok: response.ok,
          value: response.value,
          ...(!response.ok && response.error ? { error: response.error } : {}),
          wireResponse: responseWithoutDelivery,
          witness: {
        ...
        } satisfies ConnectorInvocationDelivery);
  ...
  private acceptRemoteDiagnostics(value: unknown): void {
    ...
    this.remoteDiagnostics = next;
    if (!prior || prior.pid !== next.pid) {
      this.invalidateProtocolAdmission();
      if (this.connectorOnline && connectorSupportsDurableDelivery(this.connectorVersion)) {
        this.requestProtocolAdmission();
      }
  ...
  private invalidateProtocolAdmission(): void {
    this.connectorStateEpoch += 1;
    this.protocolAdmissionIdentity = null;
  }
  ...
  private async call(
    ...
    options: { requestId?: string; deliveryWitness?: boolean } = {}
  ): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.pending.set(id, {
        ...
        mutationAuthorized: operation === 'operation_invoke' && payload.mutationAuthorized === true,
        deliveryWitness: options.deliveryWitness === true
      });
      this.socket!.send(JSON.stringify({
        schemaVersion: BRIDGE_SCHEMA,
        kind: 'command',
  ...
  async invokeOperationWithWitness(input: OperationInvocationRequest): Promise<ConnectorInvocationDelivery> {
    ...
    await this.ensureProtocolAdmission();
    const invoke = () => this.call(
        'operation_invoke',
        input as unknown as Record<string, unknown>,
        120_000,
        undefined,
        { requestId: deliveryContext.requestId, deliveryWitness: true }
      ) as Promise<ConnectorInvocationDelivery>;
    try {
      return await invoke();
    } catch (error) {
      if (!(error instanceof AppError) || error.code !== 'REMOTE.PROTOCOL_ADMISSION_REQUIRED') throw error;
      // Worker explicitly proved rejection at its platform pre-effect gate.
      // Re-admit the replacement generation, then retry the same durable id
      // exactly once; no new mutation identity is allocated.
      await this.ensureProtocolAdmission();
      return invoke();
    }
  }
  ...
  supportsDurableDelivery(): boolean {
    const admitted = this.protocolAdmissionIdentity;
    const diagnosticPid = Number(this.remoteDiagnostics?.pid || 0);
    return this.protocolCompatible && this.connectorOnline
      && connectorSupportsDurableDelivery(this.connectorVersion)
      && Boolean(admitted
        && admitted.stateEpoch === this.connectorStateEpoch
        && admitted.bridgeGeneration === this.generation
        && admitted.connectorVersion === this.connectorVersion
        && admitted.diagnosticPid === diagnosticPid
        && diagnosticPid > 0);
  }

  private requestProtocolAdmission(): void {
    void this.ensureProtocolAdmission().catch(() => {
      const expectedEpoch = this.connectorStateEpoch;
      const retry = setTimeout(() => {
        if (this.connectorStateEpoch === expectedEpoch && this.connectorOnline) this.requestProtocolAdmission();
      }, 2_000);
      retry.unref();
    });
  }

  private ensureProtocolAdmission(): Promise<void> {
    if (this.supportsDurableDelivery()) return Promise.resolve();
    if (this.protocolAdmissionTask) return this.protocolAdmissionTask;
    const expectedGeneration = this.generation;
    const expectedEpoch = this.connectorStateEpoch;
    const expectedVersion = this.connectorVersion;
    const expectedDiagnosticPid = Number(this.remoteDiagnostics?.pid || 0);
    if (!this.connectorOnline || !connectorSupportsDurableDelivery(expectedVersion)
      || !Number.isSafeInteger(expectedDiagnosticPid) || expectedDiagnosticPid <= 0) {
      return Promise.reject(new AppError(
        'REMOTE.PROTOCOL_ADMISSION_NOT_READY',
        'Current Connector process diagnostics are unavailable for exact protocol admission.',
        true
      ));
    }
    ...
    task = (async () => {
      try {
        const result = await this.call('protocol_admission', {
          schemaVersion: 'omnia.connector-protocol-admission/v1',
          coreProtocol: 'omnia.connector-delivery/v1'
        }, 15_000) as Record<string, unknown>;
        if (result.schemaVersion !== 'omnia.connector-protocol-admission-result/v1'
          || result.admitted !== true || result.version !== expectedVersion
          || !/^[a-f0-9]{48}$/u.test(String(result.executionGeneration || ''))) {
          throw new AppError('REMOTE.PROTOCOL_ADMISSION_INVALID', 'Connector returned an invalid protocol admission identity.');
        }
        if (this.connectorStateEpoch !== expectedEpoch || this.generation !== expectedGeneration
          || this.connectorVersion !== expectedVersion || !this.connectorOnline
          || Number(this.remoteDiagnostics?.pid || 0) !== expectedDiagnosticPid) {
          throw new AppError('REMOTE.PROTOCOL_ADMISSION_STALE', 'Connector identity changed during protocol admission.', true);
        }
        this.protocolAdmissionIdentity = {
          stateEpoch: expectedEpoch,
          bridgeGeneration: expectedGeneration,
          connectorVersion: expectedVersion,
          diagnosticPid: expectedDiagnosticPid,
          executionGeneration: String(result.executionGeneration)
        };
      } finally {
        if (this.protocolAdmissionTask === task) this.protocolAdmissionTask = null;
      }
    })();
    this.protocolAdmissionTask = task;
    return task;
  }
  async deliveryStatus(input: ConnectorDeliveryStatusRequest): Promise<ConnectorDeliveryStatusResult> {
    ...
    return this.call('operation_delivery_status', input as unknown as Record<string, unknown>, 30_000);
  }
  async acknowledgeDelivery(input: ConnectorDeliveryAck): Promise<{ acknowledged: true; clearedMutationCount: number }> {
    return this.call('operation_delivery_ack', input as unknown as Record<string, unknown>, 30_000);
  }
}
