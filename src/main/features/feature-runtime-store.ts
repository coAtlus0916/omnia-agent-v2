...
export const FEATURE_RUNTIME_STORE_PORT_POLICIES: Readonly<Record<string, FeatureRuntimeStorePortPolicy>> = Object.freeze({
  ...
  loadProcessingRun: { permission: 'read', owner: 'optional_current_run' },
  finishProcessingRun: { permission: 'local_write', owner: 'current_run' },
  failProcessingRun: { permission: 'local_write', owner: 'current_run' },
  ...
  inspectLegacyReturnRecovery: { permission: 'read', owner: 'same_feature_run' },
  ...
  loadReturnReconcileSpec: { permission: 'read', owner: 'current_run' },
...
export class FeatureRuntimeStore {
  ...
  private assertPortPolicy(method: string, input: unknown, context: FeatureWorkerPortContext): void {
    ...
    if (policy.owner === 'optional_current_run' && !runId) return;
    if (!runId) throw new AppError('FEATURE.STORE_OWNER_MISMATCH', `${method} requires an owned Run identity.`);
    if (policy.owner === 'current_run' || policy.owner === 'optional_current_run') {
      if (!this.core.prepare(`SELECT 1 FROM feature_runs WHERE run_id=? AND feature_id=? AND feature_version=?`)
        .get(runId, context.featureId, context.featureVersion)) {
        throw new AppError('FEATURE.STORE_OWNER_MISMATCH', 'Feature Run is not owned by the active Feature version.');
      }
      return;
    }
    if (policy.owner === 'same_feature_run') {
    ...
    const versionClause = policy.owner === 'current_command' ? 'AND r.feature_version=?' : '';
    const parameters = policy.owner === 'current_command'
      ? [commandId, runId, context.featureId, context.featureVersion]
      : [commandId, runId, context.featureId];
    if (!this.core.prepare(`
      SELECT 1 FROM feature_commands c
      JOIN feature_runs r ON r.run_id=c.run_id
      WHERE c.command_id=? AND c.run_id=? AND r.feature_id=? ${versionClause}
    `).get(...parameters)) {
  ...
  private inspectLegacyReturnRecovery(input: unknown, context: FeatureWorkerPortContext): Record<string, unknown> {
    ...
    `).all(run.run_id) as Array<Record<string,any>>;
  ...
  private recordLegacyReturnRecoveryOutcome(input: unknown, context: FeatureWorkerPortContext): Record<string, unknown> {
    ...
    if(String(request.outcome)==='not_applied'){
      ...
      if(String(row.operation_id)!==String(spec.readOperation)||String(row.operation_id)!=='omnia.create-associate.gra.reconcile.v1')throw new Error('applied requires an exact GRA read-back receipt, not a preflight observation.');
  ...
  private closeLegacyFrozenInputRecovery(input: unknown, context: FeatureWorkerPortContext): Record<string, unknown> {
    ...
    if (context.featureId !== 'omnia.recording' || context.featureVersion !== '0.3.4' || sourceFeatureVersion !== '0.3.1') {
  ...
  private createProcessingRun(input: unknown, context: FeatureWorkerPortContext): Record<string, unknown> {
    ...
    if (recovery) {
      ...
      if (
        context.featureId !== 'omnia.recording'
        || recovery.schemaVersion !== 'omnia.processing-run-frozen-input-recovery/v1'
        || recovery.sourceFeatureVersion !== '0.3.4'
      ...
      frozenInputRecovery = {
        ...
        sourceFeatureVersion: '0.3.4', externalId, connectorState: 'stopped',
  ...
  private createSuccessorProcessingRun(input: unknown, context: FeatureWorkerPortContext): Record<string, unknown> {
    if (context.featureId !== 'omnia.recording') {
      throw new Error('Frozen-input successor recovery is available only to the Recording Feature.');
    }
    ...
    if (frozen.schemaVersion !== 'omnia.recording-frozen-input/v1'
  ...
  private loadProcessingRun(input: unknown, context: FeatureWorkerPortContext): Record<string, unknown> | null {
    ...
    const row = this.core.prepare(`
      ...
      WHERE r.feature_id=? AND r.feature_version=?
        AND (${runId ? 'r.run_id=?' : "json_extract(e.details_json,'$.externalId')=?"})
      ORDER BY e.occurred_at DESC,e.rowid DESC LIMIT 1
    `).get(context.featureId, context.featureVersion, runId || externalId) as Record<string, unknown> | undefined;
    if (!row) return null;
  ...
  private failProcessingRun(input: unknown, context: FeatureWorkerPortContext): Record<string, unknown> {
    ...
    try {
      const run = this.core.prepare(`
        SELECT state,state_revision FROM feature_runs WHERE run_id=? AND feature_id=? AND feature_version=?
      ...
      const updated = this.core.prepare(`
        ...
        WHERE run_id=? AND feature_id=? AND feature_version=? AND state='processing' AND state_revision=?
      `).run(terminalState, stateRevision, message, failedAt, runId, context.featureId, context.featureVersion, run.state_revision);
  ...
  private beginPythonInputTransfer(input: unknown, context: FeatureWorkerPortContext): Record<string, unknown> {
    ...
    if (!recovery) {
      throw new Error('Recording Python input requires exact stopped Connector frozen-input evidence.');
    }
    exactRecordKeys(recovery, [
    ...
    if (
      ...
      || String(started.sourceRef || '') !== `connector-recording:${externalId}`
      || recovery.connectorState !== 'stopped'
      ...
      throw new Error('Only Recording Runs may recover uncertain Python input from Connector frozen evidence.');
    }
  ...
  private commitPythonOutputHandle(input: unknown, context: FeatureWorkerPortContext): Record<string, unknown> {
    ...
    try {
      ...
      if (!run || !['processing', 'succeeded'].includes(run.state)) {
  ...
  private prepareReturnIntent(input: unknown, context: FeatureWorkerPortContext): Record<string, unknown> {
    ...
    try {
      for (const raw of targets) {
        ...
        if (context.featureId === 'omnia.create-associate' && String(target.kind) === 'object'
  ...
  private prepareReturnCommand(input: unknown, context: FeatureWorkerPortContext): Record<string, unknown> {
    ...
    } else if (context.featureId === 'omnia.create-associate' && String(intended.kind) === 'object' && String(intended.objectType) === 'GRA') {
    ...
    if (!intent || !['frozen','commanded'].includes(intent.state) || confirmation?.decision !== 'approved' || confirmation.credential_digest !== authorityDigest
    ...
    if (intent.state === 'commanded') {
      ...
      `).get(runId,intent.intent_id,String(request.operationId||''),idempotencyKey,planDigest,requestDigest,
      ...
      if (!prepared) throw new Error('The commanded Return intent has no exact unsubmitted prepared command to resume safely.');
  ...
  private projectVerifiedReturn(input: unknown, context: FeatureWorkerPortContext): true {
    ...
    const evidence = this.core.prepare(`
      SELECT e.evidence_id,e.evidence_digest,e.receipt_id,e.payload_json,
        o.run_id,o.command_id,o.feature_id,o.feature_version,o.operation_package_digest,o.operation_id,
    ...
    const confirmation = this.core.prepare(`SELECT credential_digest,connector_id,session_generation,authority_instance_id,tenant_or_org_id,pack_id,engagement_id FROM feature_confirmations WHERE run_id=? AND plan_digest=? AND decision='approved' ORDER BY created_at DESC LIMIT 1`).get(runId, command.plan_digest) as Record<string,any>|undefined;
    const safety = this.core.prepare(`SELECT workspace_ids_json FROM workspace_safety WHERE singleton=1`).get() as {workspace_ids_json:string};
    const workspaceIds = JSON.parse(safety.workspace_ids_json) as string[];
    const authorityDigest = crypto.createHash('sha256').update(canonical({ connectorId:binding.connectorId,sessionGeneration:Number(binding.sessionGeneration),engagementId:binding.engagementId,authorityInstanceId:binding.authorityInstanceId,tenantOrOrgId:binding.tenantOrOrgId,packId:binding.packId,workspaceIds })).digest('hex');
    ...
    if (recordedReceiptId !== String(evidence.receipt_id)
      || canonical(receiptPayload) !== canonical(request.payload ?? {})
      ...
      || String(confirmation.connector_id) !== String(binding.connectorId)
      || Number(confirmation.session_generation) !== Number(binding.sessionGeneration)) {
    ...
    if (context.featureId === 'omnia.create-associate' && String(intended.kind || '') === 'object'
      && projectionKind === 'object' && String(intended.objectType || '') === 'GRA') {
      const contentIdentity = intended.contentIdentity as Record<string, unknown> | undefined;
      const contentName = String(intended.contentName || '').normalize('NFKC').trim();
      const payload = object(request.payload, 'Verified GRA projection payload');
    ...
    if (existingObjects.length + existingRelations.length > 0) {
      const expectedPayload = canonical(request.payload || {});
