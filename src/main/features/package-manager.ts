...
function assertCompatibleOperationHandoff(
  ...
  if (
    ...
    || target.manifest.sequence <= source.manifest.sequence
...
async function ensureExactSourceOperationRegistration(
  ...
  const exactIdentity = () => (
    ...
    && registration.replacedPackageDigests.length === 0
  ...
  if (!exactIdentity() || !['prepared', 'committed'].includes(registration.registrationState)) {
    throw new AppError(
      ...
      'Connector did not prove the exact active source Operation registration before handoff.'
    );
  }
...
function persistPreparedOperationHandoff(
  ...
  try {
    ...
    if (!active
      ...
      || Number(active.runtime_enabled) !== 1
...
function persistCommittedOperationHandoff(
  ...
  try {
    ...
    if (!active || active.feature_version !== source.featureVersion
      || Number(active.activation_generation) !== source.activationGeneration
      || Number(active.runtime_enabled) !== 1 || active.package_digest !== source.packageDigest) {
...
function markOperationHandoffAbortPending(
  ...
  try {
    ...
    if (!head || head.feature_version !== ledger.sourceFeatureVersion
      ...
      || Number(head.activation_generation) !== ledger.sourceActivationGeneration
...
function parseManifest(envelope: OfficialPackageEnvelope): FeatureManifest {
  ...
  if (manifest.recoveryCompatibility) {
    ...
    if (manifest.featureId==='omnia.create-associate'
      && (recovery.mode !== 'partial_close_no_reuse'
        || canonicalJson(recovery.sourceFeatureVersions)!==canonicalJson(['0.2.60']))) {
      throw new Error('Create & Associate recovery packages may recover only the exact 0.2.60 partial Return generation.');
    }
  }
...
export class FeaturePackageManager {
    ...
    const drain = async (): Promise<void> => {
      for (;;) {
        const rows = this.database.prepare(`
          ...
          ORDER BY o.created_at,o.ack_id LIMIT 32
        ...
        for (const row of rows) {
          ...
          try {
            ...
            `).run(utcNow(), utcNow(), row.ack_id);
          } catch (error) {
            this.database.prepare(`
              UPDATE connector_delivery_ack_outbox SET state='pending',last_error=?,updated_at=?
              WHERE ack_id=? AND state='sending'
            `).run(error instanceof Error ? error.message.slice(0, 500) : 'delivery acknowledgement failed', utcNow(), row.ack_id);
            retryNeeded = true;
          }
        }
  ...
  install(packageFilename: string): FeatureInstallResult {
    ...
    try {
      ...
      if (current) {
        ...
        if (sourceOperation.manifest.resourceOwner || operationPackage.manifest.resourceOwner) {
          assertCompatibleOperationHandoff(sourceOperation, operationPackage);
          if (operationPackage.manifest.resourceOwner) {
  ...
  rollback(featureId: string, targetVersion: string): FeatureInstallResult {
    ...
    if (sourceOperation.manifest.resourceOwner || targetOperation.manifest.resourceOwner) {
  ...
  private async reconcileAbortPendingOperationHandoff(
    ...
    if (!this.runtime || ledger.phase !== 'abort_pending'
      ...
      || ledger.sourcePackageDigest !== active.packageDigest
      || ledger.sourceActivationGeneration !== active.activationGeneration
      || !active.runtimeEnabled) throw new AppError(
        'FEATURE.OPERATION_HANDOFF_HEAD_CAS_MISMATCH',
  ...
  async initializeRuntime(): Promise<void> {
    ...
    for (const head of this.list()) {
      ...
      try {
        ...
        const pending = this.pendingOperationHandoffCandidate(head);
        if (pending) {
          target = pending;
          handoffSource = head;
        }
  ...
  private async startRuntime(head: ActivationHead, handoffSource: ActivationHead | null = null): Promise<void> {
      ...
      head.featureVersion,
      {
        connectorInvoke: async (input, context) => {
          ...
          if (!cachedRegistration || cachedRegistration.sessionGeneration !== sessionGeneration) {
            ...
            } else if (operationManifest.resourceOwner && registration.registrationState !== 'committed') {
              throw new AppError(
                'FEATURE.OPERATION_REGISTRATION_PROTOCOL_UNAVAILABLE',
                'The Connector does not support two-phase signed resource-owner registration.'
              );
            }
            if (operationManifest.resourceOwner && registration.replacedPackageDigests.length > 0) {
              ...
              if (!ledger
                ...
                throw new AppError(
                  'FEATURE.OPERATION_HANDOFF_LEDGER_CAS_MISMATCH',
                  'A replacement Operation registration cannot be finalized outside its exact durable Core handoff.'
                );
              }
              try {
          ...
          try {
            ...
            if (recoveryContext !== undefined) {
              ...
              else if(operationId===String(spec.readOperation||'')&&operationId==='omnia.create-associate.gra.reconcile.v1'){
        ...
        storeCall: async (method, input, context) => {
          ...
          if (['recordReturnEvidence', 'recordLegacyReturnRecoveryOutcome'].includes(method)) {
            await this.flushConnectorDeliveryAcks();
          }
    ...
    if (handoffSource) {
      ...
      try {
        await ensureExactSourceOperationRegistration(this.runtime.connector, {
