...
test('PackageManager abandons only exact missing read delivery and never replays a missing mutation journal', async (t) => {
  ...
  const workerChild = (supervisor as any).child;
  await supervisor.stop();
  if (workerChild?.connected) workerChild.disconnect();
  if (workerChild && workerChild.exitCode === null && workerChild.signalCode === null) workerChild.kill('SIGKILL');
