...
test('Risk-Control association refreshes the live concurrency token without weakening frozen identity', async () => {
  const controlId = '77777777-7777-4777-8777-777777777777';
  const staleUpdatedOn = '2026-08-06T12:00:00.000Z';
  const liveUpdatedOn = '2026-08-06T12:00:09.000Z';
  let submitted:any = null;
  const result = await createOperationHandler().run('omnia.create-associate.risk-control.associate.v1', {
    target:{targetIdentityKey:'risk-control-refresh',workspaceId},
    command:{commandId:'88888888-8888-4888-8888-888888888888',idempotencyKey:'a'.repeat(64),kind:'associate_risk_control',payload:{
      engagementId,workspaceId,riskAssessmentId,riskRiskScopeId,riskName:'RAITCOR001',controlName:'TOOL.05',riskClassification:'Higher',
      riskId,updatedOn:staleUpdatedOn,isPurgeControlHiddenData:false,
      controlRiskScopes:[{controlId,riskScopeId,assertionType:'10005',riskId,assertions:[{assertion:'10018'}]}]
    }}
  }, {binding:{engagementId},invokeStep:async(stepId:string,_query:any,body:any)=>{
    if(stepId==='risk-catalog')return{plannedResponses:[{id:riskId,riskNumber:'RAITCOR001',riskRiskScopeId,updatedOn:liveUpdatedOn}]};
    if(stepId==='risk-control-detail')return{planResponseRisk:[{id:riskId,riskNumber:'RAITCOR001',classificationType:'Higher',updatedOn:liveUpdatedOn,
      riskRiskScopes:[{id:riskRiskScopeId,riskScopeId,assertionType:'10005',assertions:['10018']}]}]};
    if(stepId==='risk-control-validation')return[{showPopup:false}];
    if(stepId==='risk-control-associate'){submitted=body;return{ok:true};}
    throw new Error(`Unexpected step ${stepId}`);
  }});
  assert.deepEqual(result,{ok:true});
  assert.equal(submitted.updatedOn,liveUpdatedOn,
    'the action must use the current Risk token after an earlier association advanced it');
});
