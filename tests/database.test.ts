...
test('migration 9 retires Local mode without projecting its cached connection and preserves audit evidence', () => {
  ...
  database.db.exec(`CREATE TABLE connection_settings(singleton INTEGER PRIMARY KEY, mode TEXT NOT NULL, remote_bridge_url TEXT NOT NULL, remote_pair_id TEXT NOT NULL, remote_token_ciphertext TEXT NOT NULL, state_version INTEGER NOT NULL, updated_at TEXT NOT NULL); INSERT INTO connection_settings VALUES(1,'local','https://agent.example.invalid/v5-bridge/','','',1,'legacy');`);
