...
function modernizeDeletePythonSidecarFixture(envelope: any): any {
  const textMembers = new Set([
    'contracts/feature-runtime.json',
  ...
  for (const member of envelope.files) {
    if (textMembers.has(member.path)) {
      const original = Buffer.from(member.contentBase64, 'base64').toString('utf8');
      const updated = original
        .replaceAll('middle/python-bridge.cjs', 'middle/delete-elements-python-bridge.cjs')
        .replaceAll('python/engine.py', 'python/delete-elements-engine.py')
        .replaceAll('./python-bridge.cjs', './delete-elements-python-bridge.cjs');
      const bytes = Buffer.from(updated);
      member.contentBase64 = bytes.toString('base64');
      member.size = bytes.length;
      member.sha256 = crypto.createHash('sha256').update(bytes).digest('hex');
    }
    if (member.path === 'middle/python-bridge.cjs') member.path = 'middle/delete-elements-python-bridge.cjs';
    if (member.path === 'python/engine.py') member.path = 'python/delete-elements-engine.py';
  }
  const implementationMember = envelope.files.find((member: any) => member.path === 'docs/IMPLEMENTATION_MAP.md');
  const documentationMember = envelope.files.find((member: any) => member.path === 'docs/manifest.json');
  assert.ok(implementationMember);
  assert.ok(documentationMember);
  const documentation = JSON.parse(
    Buffer.from(documentationMember.contentBase64, 'base64').toString('utf8')
  ) as { documents: Array<{ path: string; sha256: string }> };
  const implementationDocument = documentation.documents.find(
    (document) => document.path === 'docs/IMPLEMENTATION_MAP.md'
  );
  assert.ok(implementationDocument);
  implementationDocument.sha256 = implementationMember.sha256;
  replacePackageMember(envelope, 'docs/manifest.json', documentation);
  envelope.files.sort((left: any, right: any) => left.path.localeCompare(right.path));
  return envelope;
}
...
test('signed Feature parser strictly validates action pending presentations', () => {
  const source = modernizeDeletePythonSidecarFixture(JSON.parse(fs.readFileSync(package0319, 'utf8')) as any);
  const validPending = {
