...
const legacy018FeaturePath = path.join(repository, 'feature-packages', 'recording', 'candidates', 'recording-0.4.18.ofp');
const legacy019FeaturePath = path.join(repository, 'feature-packages', 'recording', 'candidates', 'recording-0.4.19.ofp');
const legacy019OperationPath = path.join(repository, 'feature-packages', 'recording', 'candidates', 'recording-operation-0.4.19.ofop');
const candidate020FeaturePath = path.join(repository, 'feature-packages', 'recording', 'candidates', 'recording-0.4.20.ofp');
...
const deletePackage012 = path.join(repository, 'feature-packages', 'delete-elements', 'candidates', 'delete-elements-0.1.2.ofp');
...
function seedLegacyInstalledHead(
  database: DatabaseSync,
  paths: ReturnType<typeof resolveProductPaths>,
  packagePath: string
): { envelope: ReturnType<typeof verifyOfficialPackage>; digest: string; packagePath: string } {
  const envelope = verifyOfficialPackage(JSON.parse(fs.readFileSync(packagePath, 'utf8')), 'omnia-feature');
  const packageSha = packageDigest(envelope);
  const manifestMember = envelope.files.find((member) => member.path === 'manifest.json');
  assert.ok(manifestMember);
  const manifest = JSON.parse(Buffer.from(manifestMember.contentBase64, 'base64').toString('utf8'));
  const relativePackagePath = path.posix.join(
    'packages', 'installed', envelope.packageId, envelope.version, packageSha.slice('sha256:'.length)
  );
  const installedRoot = path.join(paths.data, ...relativePackagePath.split('/'));
  unpackEnvelope(envelope, installedRoot);
  fs.writeFileSync(path.join(installedRoot, '.official-package-envelope.json'), JSON.stringify(envelope));
  const documentationPath = path.posix.join(
    'documentation', 'features', envelope.packageId, envelope.version, packageSha.slice('sha256:'.length)
  );
  const now = '2026-08-10T00:00:00.000Z';
  database.prepare(`INSERT INTO feature_registry(
    feature_id,feature_version,lifecycle,package_digest,publisher_key_id,health,activated_at
  ) VALUES(?,?,'active',?,'omnia-v5-official-feature-2026-01','legacy_runtime_loaded',?)`)
    .run(envelope.packageId, envelope.version, packageSha, now);
  database.prepare(`INSERT INTO documentation_registry(
    feature_id,feature_version,documentation_digest,lifecycle,activated_at,physical_path
  ) VALUES(?,?,'legacy-documentation','active',?,?)`)
    .run(envelope.packageId, envelope.version, now, documentationPath);
  database.prepare(`INSERT INTO feature_activation_heads(
  ...
  ) VALUES(?,?,1,0,'legacy source head awaiting exact handoff',?,?,?,?)`)
    .run(envelope.packageId, envelope.version, relativePackagePath, packageSha, now, documentationPath);
  database.prepare(`INSERT INTO feature_publisher_sequences(feature_id,highest_sequence,updated_at)
    VALUES(?,?,?)`).run(envelope.packageId, manifest.sequence, now);
  return { envelope, digest: packageSha, packagePath: relativePackagePath };
}

test('0.4.19 to 0.4.20 Operation handoff is signed, fail-closed on old Connector, and exact on a capable Connector', async () => {
  ...
  assert.match(packageScript, /43b48912e09d66a413e2af831fbc40ab22df8a09119bc8ca40ceb8b4ada47ef7/);
  ...
  const legacyFeatureEnvelope = verifyOfficialPackage(JSON.parse(fs.readFileSync(legacy018FeaturePath, 'utf8')), 'omnia-feature');
  ...
  assert.deepEqual(
    ...
    '0.4.20 must preserve the 0.4.18/0.4.19 handler bytes so the generic ABI fingerprint stays stable'
  );
  ...
  const target: any = {
    ...
    manifest: {
      schemaVersion: 'omnia.connector-operation-manifest/v1', packageId: 'omnia.recording.operation',
      version: '0.4.20', sequence: 33, featureId: 'omnia.recording',
      resourceOwner: {
  ...
  const stableOwnerSource = structuredClone(target);
  stableOwnerSource.digest = `sha256:${'d'.repeat(64)}`;
  stableOwnerSource.manifest.version = '0.4.19';
  stableOwnerSource.manifest.sequence = 32;
  assert.doesNotThrow(() => packageManagerTest.assertCompatibleOperationHandoff(stableOwnerSource, target),
    '0.4.19 to 0.4.20 continuity must use the stable owner identity without adding a fourth legacy digest');
  ...
  ownerDowngradeTarget.manifest.version = '0.4.21';
  ownerDowngradeTarget.manifest.sequence = 34;
  ...
  const handoffDatabase = new DatabaseSync(':memory:');
  handoffDatabase.exec(`CREATE TABLE feature_operation_handoffs(
    handoff_id TEXT,feature_id TEXT,source_feature_version TEXT,source_package_digest TEXT,
    source_operation_package_digest TEXT,source_activation_generation INTEGER,
    target_feature_version TEXT,target_package_digest TEXT,target_operation_package_digest TEXT,
    target_activation_generation INTEGER,registration_token TEXT,replaced_package_digests_json TEXT,
    phase TEXT,created_at TEXT,updated_at TEXT,last_error TEXT
  )`);
  ...
  await assert.rejects(
    packageManagerTest.ensureExactSourceOperationRegistration(handoffDatabase, oldConnector, {
  ...
  await assert.rejects(
    packageManagerTest.ensureExactSourceOperationRegistration(handoffDatabase, mixedStandaloneConnector, {
      featureId: 'omnia.recording', featureVersion: '0.4.18', operationPackage: source.envelope,
      validatedOperationPackage: source
    }),
    (error: any) => error?.code === 'FEATURE.OPERATION_HANDOFF_SOURCE_REGISTRATION_UNPROVEN',
  ...
  await packageManagerTest.ensureExactSourceOperationRegistration(handoffDatabase, capableConnector, {
    featureId: 'omnia.recording', featureVersion: '0.4.18', operationPackage: source.envelope,
    validatedOperationPackage: source
  });
  handoffDatabase.close();
  const prepared = await packageManagerTest.registerExactOperationHandoff(capableConnector, {
    featureId: 'omnia.recording', featureVersion: '0.4.20', operationPackage: {},
  ...
  await packageManagerTest.commitExactOperationHandoff(capableConnector, {
    featureId: 'omnia.recording', featureVersion: '0.4.20', operationPackageDigest: targetDigest,
...
test('install refuses to remove a signed source resource owner before staging or head mutation', () => {
  ...
  const database = new CoreDatabase(paths.database, cipher);
  try {
    const sourceFeature = verifyOfficialPackage(JSON.parse(fs.readFileSync(legacy019FeaturePath, 'utf8')), 'omnia-feature');
    const operationMember = sourceFeature.files.find((member: any) => member.path === 'connector-capability/operation.ofop')!;
    const sourceEnvelope = verifyOfficialPackage(
    ...
    const operationManifest = JSON.parse(Buffer.from(
      sourceEnvelope.files.find((member) => member.path === 'manifest.json')!.contentBase64, 'base64'
    ).toString('utf8'));
    const source: any = {
      envelope: sourceEnvelope, manifest: operationManifest, digest: packageDigest(sourceEnvelope),
      capabilityFingerprint: 'stable-recording-operation-abi'
    };
    assert.ok(source.manifest.resourceOwner, 'the immutable 0.4.19 source must carry the signed stable owner');
    const target = structuredClone(source);
    target.digest = `sha256:${'8'.repeat(64)}`;
    target.manifest.version = '0.4.20';
    target.manifest.sequence = 33;
    delete target.manifest.resourceOwner;
    const beforeRegistry = (database.db.prepare('SELECT COUNT(*) AS count FROM feature_registry').get() as any).count;
    const beforeHeads = (database.db.prepare('SELECT COUNT(*) AS count FROM feature_activation_heads').get() as any).count;
    assert.throws(
      () => packageManagerTest.assertCompatibleOperationHandoff(source, target),
      (error: any) => error?.code === 'FEATURE.OPERATION_OWNER_DOWNGRADE_FORBIDDEN'
    );
    assert.equal((database.db.prepare('SELECT COUNT(*) AS count FROM feature_registry').get() as any).count, beforeRegistry);
    assert.equal((database.db.prepare('SELECT COUNT(*) AS count FROM feature_activation_heads').get() as any).count, beforeHeads,
      'owner downgrade must fail before candidate registry staging or activation-head mutation');
  } finally {
    database.close();
    fs.rmSync(temporary, { recursive: true, force: true });
  }
...
test('one durable handoff fences a second install, rollback, and every head-disable mutation across restart', () => {
  ...
  try {
    ...
    const stagedInstall = manager.install(deletePackage011);
    const head = manager.list().find((item) => item.featureId === 'omnia.delete-elements')!;
    const staged = packageManagerTest.activeOperationHandoff(database.db, head.featureId)!;
    assert.equal(staged.phase, 'staged');
    assert.equal(staged.sourcePackageDigest, head.packageDigest);
    assert.equal(staged.targetPackageDigest, stagedInstall.packageDigest);
    assert.equal(staged.targetFeatureVersion, '0.1.1');
    assert.notEqual(staged.sourcePackageDigest, staged.sourceOperationPackageDigest,
      'the durable fixture must keep Feature and Operation identities separate');
    assert.throws(
      () => manager.install(deletePackage012),
      (error: any) => error?.code === 'FEATURE.OPERATION_HANDOFF_IN_PROGRESS'
    );
    assert.throws(
    ...
    assert.equal(recovered.sourceOperationPackageDigest, staged.sourceOperationPackageDigest);
...
test('release-owned Python retains validated current rows after ingest failure and purges them exactly after 24 hours', { timeout: 30_000 }, async () => {
  ...
  process.env.OMNIA_MANAGED_PYTHON_ENTRY = path.join(packageRoot, 'python', 'recording-engine.py');
  ...
  const { createPythonSidecarBridge } = require(path.join(packageRoot, 'middle', 'recording-python-bridge.cjs')) as {
...
test('release CPython atomically adopts a stale stopping session for one exact successor lineage', { timeout: 30_000 }, async () => {
  ...
  const { createPythonSidecarBridge } = require(path.join(packageRoot, 'middle', 'recording-python-bridge.cjs')) as any;
  const bridge = createPythonSidecarBridge({ ports: { events: { emit: async () => undefined } } });
  try {
...
test('0.4.20 loads an installed 0.4.19 legacy-path source and stages its exact handoff without closing a retryable plan', {
  skip: !fs.existsSync(candidate020FeaturePath)
}, () => {
  const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'omnia-v5-recording-upgrade-recovery-'));
  const paths = resolveProductPaths(temporary);
  const database = new CoreDatabase(paths.database, cipher);
  const legacyInstalled = seedLegacyInstalledHead(database.db, paths, legacy019FeaturePath);
  const legacyRunId = '60664a82-102b-42e4-9163-78009fd8aa1f';
  const exactRecordingId = '53c4f730-4419-4a30-9da8-72dcafe0324c';
  ...
  `).run(legacyRunId, crypto.randomUUID(), 'omnia.recording', '0.4.19', engagementId, 'a'.repeat(64), createdAt, createdAt);
  database.db.prepare(`
    INSERT INTO feature_run_events(event_id,run_id,revision,from_state,to_state,event_type,details_json,occurred_at)
    VALUES(?,?,1,'','processing','run.processing_started',?,?)
  `).run(crypto.randomUUID(), legacyRunId, JSON.stringify({
  ...
  try {
    ...
    const installed = manager.install(candidate020FeaturePath);
    assert.equal(installed.featureVersion, '0.4.20');
    assert.match(installed.runtimeReason, /staged/u);
    assert.equal(manager.list().find((item) => item.featureId === 'omnia.recording')?.featureVersion, '0.4.19',
      'the installed legacy source head remains authoritative until Connector handoff commits');
    const handoff = packageManagerTest.activeOperationHandoff(database.db, 'omnia.recording')!;
    assert.equal(handoff.phase, 'staged');
    assert.equal(handoff.sourceFeatureVersion, '0.4.19');
    assert.equal(handoff.sourcePackageDigest, legacyInstalled.digest);
    assert.equal(handoff.targetFeatureVersion, '0.4.20');
    assert.equal(handoff.targetPackageDigest, installed.packageDigest);
    assert.notEqual(handoff.sourcePackageDigest, handoff.sourceOperationPackageDigest);
...
test('recording 0.4.18 remains an immutable verified legacy-path candidate and cannot be freshly installed as a new package', () => {
  ...
  const envelope = verifyOfficialPackage(JSON.parse(fs.readFileSync(legacy018FeaturePath, 'utf8')), 'omnia-feature');
  ...
  assert.equal(runtimeContract.storePorts.includes('appendEvidence'), false,
    'the immutable 0.4.18 bytes predate the explicit appendEvidence contract and must not be rewritten');
  assert.equal(runtimeContract.pythonSidecar.bridgePath, 'middle/python-bridge.cjs');
  assert.equal(runtimeContract.pythonSidecar.entryPath, 'python/engine.py');
  ...
  for (const member of runtimeContract.pythonSidecar.members) {
    assert.equal(fs.statSync(path.join(unpacked, ...member.split('/'))).isFile(), true,
      `immutable candidate member ${member} must remain present under its signed legacy path`);
  }
  assert.equal(fs.statSync(path.join(unpacked, ...runtimeContract.pythonSidecar.bridgePath.split('/'))).isFile(), true);
  ...
  try {
    const manager = new FeaturePackageManager(database.db, paths);
    assert.throws(
      () => manager.install(legacy018FeaturePath),
      /Feature Python sidecar runtime policy is invalid/u,
      'grandfathering applies only when reading an already-installed source head, never to a fresh candidate install'
    );
    assert.equal(manager.list().some((item) => item.featureId === 'omnia.recording'), false);
  } finally {
    database.close();
    fs.rmSync(temporary, { recursive: true, force: true });
  }
...
test('recording 0.4.19 immutable Feature and standalone Operation candidates are identical at the embedded boundary', {
  skip: !fs.existsSync(legacy019FeaturePath) || !fs.existsSync(legacy019OperationPath)
}, () => {
  const featureBytes = fs.readFileSync(legacy019FeaturePath);
  const operationBytes = fs.readFileSync(legacy019OperationPath);
  ...
  try {
    ...
    assert.equal(digest(packagedHandler), '43b48912e09d66a413e2af831fbc40ab22df8a09119bc8ca40ceb8b4ada47ef7');
    ...
    assert.equal(runtime.pythonSidecar.bridgePath, 'middle/python-bridge.cjs');
    assert.equal(runtime.pythonSidecar.entryPath, 'python/engine.py');
    assert.equal(runtime.storePorts.includes('appendEvidence'), false,
      'the immutable 0.4.19 candidate remains a legacy installed-source fixture and must not be rewritten');
