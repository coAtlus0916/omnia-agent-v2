...
const candidate = path.join(repository, 'feature-packages', 'workpaper-preparation', 'candidates', 'workpaper-preparation-0.1.3.ofp');
const operationCandidate = path.join(repository, 'feature-packages', 'workpaper-preparation', 'candidates', 'workpaper-preparation-operation-0.1.3.ofop');
...
test('workpaper candidate is immutable Feature-only and installs in isolation', () => {
  ...
  try {
    ...
    assert.equal(envelope.version, '0.1.3');
    ...
    assert.equal(operation.version, '0.1.3');
    ...
    assert.deepEqual(fs.readFileSync(path.join(unpacked, 'middle', 'worker.cjs')),
      Buffer.from(fs.readFileSync(path.join(source, 'middle', 'worker.cjs'), 'utf8').replaceAll('__FEATURE_VERSION__', '0.1.3')));
    assert.deepEqual(fs.readFileSync(path.join(unpacked, 'python', 'engine.py')), fs.readFileSync(path.join(source, 'python', 'engine.py')));
    ...
    try {
      ...
      assert.equal(installed.featureVersion, '0.1.3');
...
test('release CPython 3.13.14 builds a deterministic Tab 201 to Tab 209 plan', () => {
  ...
  const result = spawnSync(releasePython, ['-I', '-S', '-E', path.join(source, 'python', 'engine.py'), '--self-check'], {
...
test('source contract is no-replay and leaves Connector business-free', () => {
  ...
  assert.equal(crypto.createHash('sha256').update(fs.readFileSync(candidate)).digest('hex'), 'fbe14952f4cc5bfb5679147db64746ee105dee0d3e3b34a6a4592198ecf4d7bf');
});
...
test('Feature action chain reaches exactly one real mutation and receipt-backed projection', async () => {
  ...
  process.env.OMNIA_MANAGED_PYTHON_ENTRY = path.join(source, 'python', 'engine.py');
