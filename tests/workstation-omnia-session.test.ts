...
test('Omnia origin allowlist rejects arbitrary hosts and extracts only canonical engagement routes', () => {
  assert.equal(isAllowedOmniaUrl('https://omnia.example.invalid/engagement/x'), true);
  assert.equal(isAllowedOmniaUrl('https://evil.example/engagement/x'), false);
  assert.equal(
    parseEngagementId(`https://omnia.example.invalid/engagement/${engagementId}/home`),
    engagementId
  );
...
test('target binding rejects multiple Pack pages instead of selecting the first', () => {
  assert.throws(
    () => _test.selectUniqueTargetIndex([
      `https://omnia.example.invalid/engagement/${engagementId}/home`,
      'https://omnia.example.invalid/engagement/aaaaaaaa-aaaa-0000-0000-aaaaaaaaaaaa/home'
    ]),
...
test('Authorization is bound to the exact target Engagement and rejects identity drift', () => {
  ...
  const target = `https://omnia.example.invalid/engagement/${first}/home`;
  assert.deepEqual(
    _test.authorizationEngagementId(`https://api.example.invalid/work/v1/engagements/${first}/items`, target),
    { engagementId: first, identityMismatch: false }
  );
  assert.deepEqual(
    _test.authorizationEngagementId(`https://api.example.invalid/engagements/v1/${second}/headers/hierarchy`, target),
...
test('live hierarchy snapshot exposes only explicit canonical authority identities', async()=>{
  ...
  const page={url:()=>`https://omnia.example.invalid/engagement/${engagementId}/home`};
  ...
  try{
    ...
    (connector as any).authByPage.set(page,{headers:{authorization:'Bearer live'},apiOrigin:'https://api.example.invalid',engagementId,identityMismatch:false});
    (connector as any).api=async()=>[{engagementId,name:'Live Pack',clientName:'Client',tenantId,packId}];
    const status=await connector.status();assert.equal(status.status,'connected');assert.equal(status.authorityInstanceId,'https://api.example.invalid');assert.equal(status.tenantOrOrgId,tenantId);assert.equal(status.packId,packId);
...
test('authority extraction uses the explicit hierarchy Engagement ID as the Pack identity and never display names',()=>{
  assert.deepEqual(_test.canonicalAuthorityIdentity('https://api.example.invalid',{engagementId,name:'Display Pack',clientName:'Display Client'}),{
    authorityInstanceId:'https://api.example.invalid',tenantOrOrgId:'',packId:engagementId
  });
});

test('SSO/new-tab handoff accepts one Pack only in the bound browser context', () => {
  const pack = `https://omnia.example.invalid/engagement/${engagementId}/home`;
  assert.equal(_test.selectSafeTargetIndex([
    { url: 'https://omnia.example.invalid/login', contextId: 1 },
  ...
  assert.equal(_test.selectSafeTargetIndex([
    { url: 'https://login.example.invalid/tenant/oauth2/authorize', contextId: 1 },
    { url: pack, contextId: 1 }
  ], 0), 1);
});
...
test('external enterprise IdP navigation remains waiting_login and is never trusted as a Pack', async () => {
  ...
  const page = { url: () => 'https://login.example.invalid/tenant/oauth2/authorize' };
  try {
...
test('refresh is a passive live status probe that preserves process, session, target, and Pack identity', async () => {
  ...
  const targetUrl = `https://omnia.example.invalid/engagement/${engagementId}/home`;
  ...
  try {
    ...
    (connector as any).identify = async () => {
      ...
      if (identityReads === 1) return {
        ...
        authorityInstanceId: 'https://api.example.invalid',
...
test('explicit connect recaptures Authorization on an existing target without reload or navigation', async () => {
  ...
  const page = {
    url: () => `https://omnia.example.invalid/engagement/${engagementId}/home`,
    isClosed: () => false,
